...
mysql_select_db($database_conex, $conex);
$query_consulta_hitos = "SELECT  hitos.id_hito, hitos.archivo, hitos.tipo_archivo, hitos.id_estado, categorias.nombre_categoria,  estados.nombre_estado,  funcionarios.nombre, funcionarios.paterno, funcionarios.materno, prioridad, asunto,  fecha_reunion,  fecha_plazo  FROM  hitos, categorias, estados, funcionarios WHERE categorias.id_categoria=hitos.id_categoria AND estados.id_estado=hitos.id_estado AND funcionarios.id_funcionario=hitos.id_funcionario ORDER BY fecha_plazo DESC";
 ...
 <tbody>
   ...
   ?>
     ...
     <div class="tooltip-inner">    
	   ...
	   if($row_consulta_hitos['id_estado']<5){
		     
	   if ($diferencia>=8) {
   		   echo '<div class="alert alert-success">'."VIGENTE".'<br>'.utf8_encode("d&iacuteas restantes: ".$diferencia.'</div>');
		   $estado=1;
		   mysql_select_db($database_conex, $conex);
           $query_actualiza = "UPDATE hitos SET id_estado='$estado' WHERE id_hito='$row_consulta_hitos[id_hito]'";
...
<strong><i class="fas fa-download"></i> Descarga de archivos</strong>
<br><br>
<a href="actas/formato_acta_hitos_serviu.pdf" title="Descargar formato acta de reuni&oacute;n en PDF" target="_new">
<i class="fas fa-file-pdf"></i> Formato acta de reuni&oacute;n en PDF</a><br>
<a href="actas/formato_acta_hitos_serviu.docx" title="Descargar formato acta de reuni&oacute;n en WORD">
<i class="fas fa-file-word"></i> Formato acta de reuni&oacute;n en WORD</a></div>
